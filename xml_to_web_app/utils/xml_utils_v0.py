...
def create_cdata_section(content: str) -> str:
    """
    Crée une section CDATA pour du contenu HTML.
    
    Args:
        content: Contenu HTML à encapsuler
        
    Returns:
        Section CDATA
    """
    # Échappe les séquences CDATA imbriquées si nécessaire
...
def validate_html_content(content: str) -> None:
    ...
    Raises:
        PresentationValidationError: Si le contenu HTML n'est pas valide
    """
    if content is None:
    ...
    if len(content) > 100000:  # 100KB max
        raise ValidationError("Le contenu de présentation ne peut pas dépasser 100 000 caractères")
